...
namespace SRML.Editor
{
	...
	public class BundleFieldReplacer : ScriptableObject, IFieldReplacer
	{
		...
		public ICollection<IFieldReplacement> FieldReplacements =>
			fieldReplacements.Select((x) => (IFieldReplacement)x).ToList();
	}
	...
	[Serializable]
	public struct
		BundleFieldReplacement : IFieldReplacement //if this isnt a scriptable object unity absolutely refuses to serialize it correctly
	{
		[SerializeField]
